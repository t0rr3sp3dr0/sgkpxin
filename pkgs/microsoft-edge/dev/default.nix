...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url = "https://officecdnmac.microsoft.com/pr/${vars.guid}/MacAutoupdate/MicrosoftEdgeDev-${vars.version}.pkg";
    sha256 = "${vars.hash}";
  };
  ...
  unpackPhase = ''
    ...
    zcat -vf './MicrosoftEdgeDev-${version}.pkg/Payload' | cpio -vi
  ...
  installPhase = ''
    ...
    mv -vf './Microsoft Edge Dev.app' "''${out}/Applications"
    makeWrapper "''${out}/Applications/Microsoft Edge Dev.app/Contents/MacOS/Microsoft Edge Dev" "''${out}/bin/${pname}"
