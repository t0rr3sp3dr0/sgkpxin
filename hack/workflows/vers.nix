let
  ...
  newElem = cur: old: system:
    {
      name = system;
      value = ( cur.${ system } or { } ) // ( old.${ system } or { } );
    };

  newVers = cur: old:
    listToAttrsMap ( newElem cur old ) ( builtins.attrNames ( cur // old ) );
in
  builtins.toJSON (
    let
      package = builtins.replaceStrings [ "\r" "\n" ] [ "" "" ] ( builtins.readFile "/dev/stdin" );
    in
      newVers ( curVers package ) ( oldVers package )
  )
